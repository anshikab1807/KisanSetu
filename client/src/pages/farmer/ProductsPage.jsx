...
import { getFarmerProducts, deleteProduct } from "../../redux/slices/productSlice";
...
const ProductsPage = () => {
  ...
  if (loading && farmerProducts.length === 0) return <Loader />;
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-gradient-to-br from-green-50 via-green-100 to-green-200 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-2 text-green-800">My Products</h1>
          <p className="text-gray-600">Manage your product inventory</p>
        </div>
        <Link
          to="/farmer/products/add"
          className="mt-4 md:mt-0 btn btn-primary flex items-center space-x-2 bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors"
        >
          <FaPlus />
      ...
      <div className="mb-8">
        <div className="relative max-w-md">
          <input
            ...
            className="w-full px-4 py-2 pl-10 border border-green-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
          />
      ...
      {filteredProducts.length > 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-green-100">
                <tr>
                  {["Product", "Price", "Quantity", "Category", "Status", "Actions"].map(
                    (header) => (
                      <th
                        key={header}
                        className="text-left px-6 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider"
                      >
                        {header}
                      </th>
                    )
        ...
        <div className="text-center py-12 glass rounded-xl bg-white/70 shadow-lg">
          ...
          <Link
            to="/farmer/products/add"
            className="btn btn-primary bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors"
          >
